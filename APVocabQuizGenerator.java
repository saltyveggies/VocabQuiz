...
public class APVocabQuizGenerator {
    ...
    public static void main(String args[]) {
        ...
        while (true) {
            ...
            u = scan.nextLine().replace(" ", "");

            while (!(u.equals("0") || u.equals("A1") || u.equals("A2") || u.equals("A3") || u.equals("A4") || 
                u.equals("A5") || u.equals("A6") || u.equals("A7") || u.equals("B1") || u.equals("B2") || u.equals("B3") ||
                u.equals("B4") || u.equals("B5") || u.equals("B6") || u.equals("C1") || u.equals("C2") || u.equals("C3") || 
                u.equals("C4") || u.equals("C5") || u.equals("C6") || u.equals("D1") || u.equals("D2") || u.equals("D3") || 
                u.equals("D4") || u.equals("D5") || u.equals("D6")) )  {
                System.out.println("Entrada inválida. Reentre la unidad.");
                u = scan.nextLine();
            }
            if (u.equals("0")) {
                break;
            ...
            nCheck = scan.nextLine();
            
            
            
            //determines size of APVocabList requested
            int size = 0;
            for (APVocabList apvl : masterList) {
                if (apvl.getName().equals(u)) {
                    size = apvl.getLength();
                }
            }
            ...
            //Makes sure the input is numeric (either a double or an int), and less than or equal to the size of the APVocabList
            //the boolean of this while statement shouldn't throw an error, because the second condition
            //will only be checked if String nCheck IS numeric
            while (!isNumeric(nCheck) || Double.valueOf(nCheck) < 0 || Double.valueOf(nCheck) > size) {
                System.out.println("Entrada inválida. Reentre el número.");
                if (isNumeric(nCheck) && Double.valueOf(nCheck) > size) {
                    System.out.println("Tamaño de la unidad: " + size);
                }
                nCheck = scan.nextLine();
            }
            
            
            //puts the String input from nCheck into a double, ndoub.
            //casts the double to an int (if user inputs 2.3, it will assume 2 is the intention)
            ndoub = Double.valueOf(nCheck);
            n = (int) ndoub;
    ...
    public static void printUnitQuestions (APVocabList apvl, int num) { 
        ...
        /**
         * BUG PATCHED: this line of code assigns all indices in the array to value -1 first. Without this line of code,
         * the value 0 would never be added to the array, because int arrays are initialized with default values of 0,
         * and the method refuses to add 0 to the array if it's already found.
         * 
         * We're  setting all values of arr to -1 at first because -1 is an example of a number outside the range of values 
         * generated by the Math.random.
         * --> This way, no index will get skipped over
         * 
         */
